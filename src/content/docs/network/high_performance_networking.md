...
セグメンテーションをNICにオフロードすることで、1パケットのサイズを大きくすることができる。
それにより、プロトコルスタックで処理されるパケットの数を減らすことができるため、CPUの負荷を軽減することができる。
トランスポートレイヤのプロトコルはIPレイヤで許容される最大のサイズ(64KB程度)までのデータを1つのIPパケットで送信することができる。

[Segmentation Offloads](https://www.kernel.org/doc/html/next/networking/segmentation-offloads.html)
